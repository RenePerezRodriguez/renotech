/**
 * ============================================================
 *  SCRIPT DE CARGA MULTIPLE V2 — RENOTECH
 *  carga_multiple_v2.js
 * ============================================================
 *
 *  ¿QUÉ HACE?
 *  Procesa los 5 Excels de "PARA SCRIPT SEGUNDA VERSION" (compras 11-15)
...
const SOURCE_DIR = 'C:/Users/Rene_/Downloads/PARA SCRIPT SEGUNDA VERSION';
const GUIA_FILE  = 'SEGUNDA GUIA COMPRAS SCRIPT.xlsx';
...
async function leerGuia() {
  ...
  const ordinales = {
    11: ['DECIMA PRIMERA', 'DÉCIMA PRIMERA'],
    12: ['DECIMA SEGUNDA', 'DÉCIMA SEGUNDA'],
    13: ['DECIMA TERCERA', 'DÉCIMA TERCERA'],
    14: ['DECIMA CUARTA', 'DÉCIMA CUARTA'],
    15: ['DECIMA QUINTA', 'DÉCIMA QUINTA'],
  };
  const archivosEnDir = fs.readdirSync(SOURCE_DIR).filter(f => f.toLowerCase().endsWith('.xlsx') && !f.includes('GUIA') && !f.toUpperCase().includes('DEVOLUCION'));
  const archivosPorNumero = {};
  for (const [num, prefixes] of Object.entries(ordinales)) {
    const found = archivosEnDir.find(f => {
      const upper = f.toUpperCase();
      return prefixes.some(p => {
        // Normalize spaces for matching (handles double spaces in filenames)
        const normUpper = upper.replace(/\s+/g, ' ');
        const normP = p.replace(/\s+/g, ' ');
        return normUpper.startsWith(normP + ' ') || normUpper.startsWith(normP + '\t');
      });
    });
  ...
  for (let r = 2; r <= sheet.rowCount; r++) {
    ...
    if (!numero) {
      // Skip non-numeric rows (e.g. DEVOLUCION — handled by separate script)
      const rawVal = String(row.getCell(1).value || '').trim().toUpperCase();
      if (rawVal.includes('DEVOLUCION')) console.log('    ⏭️  Fila DEVOLUCION detectada — se omite (script separado)');
      continue;
    }
...
async function leerExcelCompra(excelPath) {
  ...
  // Known header keywords to detect if row 1 is a real header or a merged title
  const KNOWN_HEADERS = ['ID','CODIGO FABRICA','CODIGO OE','DESCRIPCION','MARCA','COSTO','CANTIDAD','STOCK','PRECIO','ORIGEN','TOTAL'];

  // Detect header row: check if row 1 has known column names
  let headerRow = 1;
  const row1vals = [];
  sheet.getRow(1).eachCell({ includeEmpty: true }, (cell) => {
    row1vals.push(String(cell.value == null ? '' : cell.value).trim().toUpperCase());
  });
  const hasKnownHeaders = row1vals.some(h => KNOWN_HEADERS.some(kh => h.includes(kh)));
  if (!hasKnownHeaders && sheet.rowCount > 1) {
    headerRow = 2;
    console.log('    \u2139\uFE0F  Fila 1 es t\u00EDtulo fusionado, usando fila ' + headerRow + ' como headers');
  }
  ...
  for (let r = headerRow + 1; r <= sheet.rowCount; r++) {
    ...
    const stock            = parseNum(findVal(rd, ['CANTIDAD','STOCK','EXISTENCIA','SALDO']));
...
async function obtenerOcrearEmpresaYCuenta(nombre, hqBranchId) {
  ...
  if (!empSnap.empty) {
    ...
    const data = {
      nombre,
      notas: 'Creada por carga_multiple_v2.js durante migración V2',
      isActive: true,
      cuentaCount: 0,
      saldoTotal: 0,
...
async function cargarCompraEnHQ(productos, hqBranch, compra, cuentaInfo) {
  ...
  const importBatchId = `migration_multi_v2_${compra.numero}_${Date.now()}`;
...
async function main() {
  console.log('\n╔═══════════════════════════════════════════════════════════╗');
  console.log('║  RENOTECH — CARGA MÚLTIPLE V2 — COMPRAS 11-15 (carga_multiple_v2.js) ║');
  console.log('╚═══════════════════════════════════════════════════════════╝');
