...
import { Eye, Printer, ShoppingBag, Download, RotateCcw, User, AlertTriangle, CornerUpLeft, TrendingUp, Calendar, X } from 'lucide-react';
...
export default function SalesHistoryPage() {
    ...
    const filteredSales = useMemo(() => {
        return sales.filter(sale => {
            ...
            const matchesSearch = !searchTerm ||
                ...
                (sale.cliente?.nit || '').includes(normSearch);

            const saleDate = ensureDate(sale.fecha);
            // Fix timezone issue by comparing date strings or timestamps carefully
    ...
    const usersList = useMemo(() => {
        ...
        sales.forEach(s => {
            ...
            if (userId && !usersMap.has(userId)) {
                usersMap.set(userId, (formatUserName(s.usuarioNombre) || '—').toUpperCase());
            }
        });
    ...
    const exportToCSV = () => {
        ...
        const rows = filteredSales.map(s => {
            ...
            return [
                ...
                s.cliente?.nit || 'S/N',
                s.cliente?.telefono || '---',
                (formatUserName(s.usuarioNombre) || s.usuarioId || '').toUpperCase(),
                branchName,
                s.metodoPago || 'S/D',
                s.total.toFixed(2),
    ...
    return (
        <>
            ...
            {/* Table / List Area */}
            <div className="bg-white dark:bg-background border border-slate-200 dark:border-white/10 rounded-3xl shadow-xl overflow-hidden flex flex-col transition-all">
                        ...
                        {paginatedSales.map((sale) => (
                            <div key={sale.id} onClick={() => setSelectedSale(sale)} className={clsx(
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="px-2.5 py-1 bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-xl text-[9px] font-black text-slate-500 dark:text-slate-400 uppercase">
                                        {sale.metodoPago || 'EFE'}
                                    </div>
                                </div>
                            </div>
                            ...
                            {paginatedSales.map((sale) => (
                                <tr key={sale.id} className={clsx(
                                    ...
                                    <td className="px-6 py-4 text-center">
                                        <span className="px-2 py-1 bg-slate-100 dark:bg-white/5 rounded text-[9px] font-black text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-white/10 uppercase tracking-tighter">
        ...
        {/* Detail Modal - Portal & Suite Pro Styling */}
            {selectedSale && typeof document !== 'undefined' && createPortal(
                (() => {
                    ...
                    return (
                        <div className="fixed inset-0 z-1000 flex items-end md:items-center justify-center bg-black/60 backdrop-blur-sm p-0 md:p-4 animate-in fade-in duration-200">
                            <div 
                            ...
                            >
                                ...
                                <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar dark:bg-background/50">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="border border-slate-200 dark:border-white/10 rounded-2xl overflow-hidden shadow-sm relative min-h-25">
                                            {loadingItems && (
                                    ...
                                    <div className="bg-slate-50 dark:bg-black/20 p-4 rounded-2xl flex items-center justify-between text-[9px] font-black text-slate-400 uppercase tracking-widest border border-slate-100 dark:border-white/10">
                                        <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-emerald-500" /><span>Agente: {(formatUserName(sale.usuarioNombre) || 'SISTEMA_CORE').toUpperCase()}</span></div>
